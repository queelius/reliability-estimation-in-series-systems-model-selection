...
\item Given the system failure time and the true failed component, the masking is non-informative (i.e., all candidate sets containing that component are equally likely).
\end{enumerate}
...
\begin{theorem}[Series System Reliability Function]
...
\begin{theorem}[Series System Hazard Function]
...
Applying Theorems \ref{thm:sys_reliability_function}, \ref{thm:sys_failure_rate}, and \ref{thm:sys_lifetime} to series systems with Weibull-distributed components yields specific analytical forms for the system lifetime distribution.
...
The shape parameter determines the failure characteristics. We vary the shape paramenter of component 3 from $0.1$ to $3.5$ and observe the effect it has on the MLE. When $k_3 < 1$, this indicates infant mortality, and when $k_3 > 1$, this indicates wear-out failures.
...
\section{Weibull Series Homogenous Shape Model}

In the sensitivity analysis in Section~\ref{sec:simulation-study}, we found that the MLE is very sensitive to deviations from a well-designed system. In this section, we develop a reduced model that assumes homogeneity in the shape parameters of the components, which simplifies analysis and reduces estimator variability.
...
In order to determine if a reduced model (e.g., Weibull series system in which all of the shape parameters are homogeneous) is appropriate, a hypothesis test test may be conducted to determine if there is statistically significant evidence in support of the null hypothesis $H_0$, e.g., that all of the shape parameters are homogeneous.
...
\subsection{Simulation Study: Full Weibull Model vs Reduced (Homogenous Shape) Model}
...
Figure \ref{fig:lrt-contour} provides a contour plot with varying sample sizes along the $x$-axis, shape of component 3 along the $y$-axis, and median $p$-value along the color scale. The contour lines corresponding to $p$-values of $0.05$ and $0.1$ are often used as a threshold for statistical significance. Points outside of these contours in dark blue are indicative of significant evidence against the null model and points inside of these contours in light blue for $0.1$ and green for $0.05$ are indicative of insufficient evidence against the null model.
...
\item In Figure \ref{fig:lrt-samp-size}, we fix $k_3$ at $1.1308$ and vary the sample size. The median $p$-value only manages to drop below the $0.05$ theshold with sample sizes around $10000$. In the more stringent criterion given by the $95$th percentile of the $p$-values, nearly $30000$ observations are necessary to reject the null hypothesis in $95\%$ of the simulations.
\end{itemize}

\subsubsection*{Sensivity to Shape Parameter ($k_3$)}
