...
- **2026-06-14**: v4 REWORK EXECUTED end to end (paper/paper.tex rewritten from the 11pp condensed draft to a 16pp single-column draft; builds clean, 0 undefined refs/citations, all bib entries now cited). Proof phase: the zero-first-order-gain result is now a PROVEN proposition (the "conjugacy identity" I^{-1} grad g in V), general-m and general-scales for complete data via the (k, eta) 2D-closure argument; masked/censored case reduced to four block proportionalities verified to MC precision (notes/verify-structural-lemma.py). KEY UNIFICATION: the same identity yields BOTH zero variance gain (Corollary 1, the MSE result) AND zero linear bias term (Corollary 2, bias = O(CV^2)); they are one fact. Full derivation in notes/zero-first-order-gain.md; masked verification ncp constant c_D=0.487 (theory) vs 0.50 (empirical), population bias c_B=0.28 with linear term confirmed zero (+0.002% at CV=0). New figures from existing data (no new sims): decoupling_ncp_collapse.pdf (power collapses on n*CV^2; CV50~3.5/sqrt(n)), population_bias.pdf (separates specification from estimation bias). New tables: MSE decomposition to n=5000 with paired t, R(t) bias, AIC/BIC pretest comparators. Citation hygiene done: stale towell2023reliability dropped, towell2025masked + towell2025weibull-fim + ~20 prior-art entries (FIC, pretest, McCool, Hart, Aitchison-Silvey, van der Vaart, etc.) added and all wired in. Structure: added Section 5 Theoretical Analysis (proposition + 2 corollaries + decoupling + FIC engagement) and Appendix D (proof); reframed Section 6 as pretest evaluation (Bancroft/Judge-Bock); fixed C1-C2-C3 statement (NEW-C1), AIC mechanism (NEW-M3), uniqueness characterization (NEW-M6). REMAINING before submission: (a) mechanical IEEEtran 4-column conversion (~16pp single-col compresses to ~9-10pp two-col); (b) optional fully-analytic masked-case proof; (c) Zenodo DOI mint. All 2026-06-10 review CRITICAL + MAJOR findings addressed; verify with a re-review pass.
