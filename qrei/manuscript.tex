...
The full model is more flexible but harder to estimate with limited masked data. The reduced model is more parsimonious and---uniquely among single-parameter reductions---renders the system lifetime itself Weibull \citep{barlow1975}, enabling closed-form reliability metrics. But is the simplification safe? The existing literature on this question focuses almost entirely on whether a likelihood ratio test can \emph{detect} the difference between models \citep{towell2023reliability}. This is the wrong question. The right question is: when the reduced model is wrong, does it produce predictions that are wrong enough to change an engineering decision?

\subsection{Related Work}

Maximum likelihood estimation from masked series system data has been developed through frequentist \citep{Usher-1988, Lin-1993, Joh-1989} approaches. \citet{Huairu-2013} provide the baseline system configuration used in our simulations, and \citet{towell2023reliability} developed the likelihood model incorporating both right-censoring and candidate sets that we use throughout.

The Weibull closure property under common shape is classical \citep{barlow1975}. \citet{Craiu-2005} addressed model selection for competing-risks models with masking, but not in the Weibull series system context. The model selection question---common shape versus heterogeneous shapes---has not, to our knowledge, been studied systematically for masked data, nor has the consequence of misspecification been quantified. We employ the likelihood ratio test \citep{wilks1938} and compare with AIC \citep{akaike1974} and BIC \citep{schwarz1978}.
...
\item \textbf{Masking}: For failed systems, only a candidate set $C_i \subseteq \{1, \ldots, m\}$ of possible failure causes is observed, not the true failed component $K_i$. The candidate set always contains the true cause ($K_i \in C_i$), and the masking mechanism is non-informative about $\v\theta$ \citep{towell2023reliability}.
\end{itemize}
...
\end{equation}
which is maximized numerically using the L-BFGS-B algorithm \citep{byrd1995} with analytical gradients. For details, see \citep{towell2023reliability}.
...
\begin{property}[Weibull Closure {\citealt{barlow1975}}]
...
We measure departure from shape homogeneity by the coefficient of variation $\text{CV}_k = \text{sd}(k_1,\ldots,k_m)/\text{mean}(k_1,\ldots,k_m)$. Throughout this paper, we use a 5-component baseline system from \citep{Huairu-2013} with $\text{CV}_k \approx 4\%$:
...
All simulations use masking probability $p = 0.215$ and censoring quantile $q = 0.825$. To vary shape heterogeneity, we generate $m$ shapes uniformly spaced about a mean of 1.18 with a specified target CV. Because the shapes are discrete, the realized CV differs slightly from the target (e.g., target CV $= 10\%$ yields actual CV $\approx 13.7\%$). All tables and figures report the actual CV; approximate values are used in prose for readability. All simulations use the \texttt{wei.series.md.c1.c2.c3} R package \citep{towell2023weibull} with L-BFGS-B optimization \citep{byrd1995} and analytical gradients. A sensitivity analysis in \citep{towell2023reliability} shows that MLE precision depends strongly on component failure probability and that shape parameters are inherently harder to estimate than scale parameters---even without masking or censoring---both observations motivating the common-shape reduction studied here.
...
\end{equation}
which is asymptotically $\chi^2_{m-1}$ under the null hypothesis $H_0: k_1 = \cdots = k_m$ \citep{wilks1938}.
...
Figure~\ref{fig:lrt-vs-aic-bic} compares the LRT with AIC \citep{akaike1974} and BIC \citep{schwarz1978}. Under perfect homogeneity (CV $= 0$), the LRT is well-calibrated (4.6--6.8\%), AIC is liberal (8.2--12.4\%, with false positive rate increasing with $n$ because its fixed penalty does not scale), and BIC is over-conservative (${\leq}\,0.2\%$), making it unsuitable for detecting the subtle heterogeneity characteristic of well-designed systems.
